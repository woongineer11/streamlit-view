...
st.markdown('표 넣기')
st.markdown('- RMSE와 Corr 측면에서 다른 모델에 비해 XGB와 NNet이 비슷하게 좋은 성능을 보임')
st.markdown('- RMSE측면에서 $Y_3$, $Y_4$, $Y_6$의 경우 XGB가 가장 좋은 성능을 보임')
st.markdown('- 다른 변수들과 달리 $Y_4$의 경우 NNet에서 가장 좋은 성능을 보이지만, XGB에서의 성능과 비슷함')
st.markdown('- XGB를 사용했을 때, backward elimination으로 선택된 변수는 다음과 같음')
st.markdown('- $Y_3$의 경우 $X_1$, $X_3$, $X_4$, $X_5$, $X_6$, $X_7$, $X_8$, $X_{10}$')
st.markdown('- $Y_4$의 경우  $X_1$, $X_3$, $X_4$, $X_5$, $X_6$, $X_7$, $X_8$, $X_{10}$')
st.markdown('- $Y_5$의 경우 $X_1$, $X_3$, $X_4$, $X_5$, $X_6$, $X_7$, $X_{10}$, $X_{11}$')
st.markdown('- $Y_6$의 경우 $X_1$, $X_3$, $X_4$, $X_6$, $X_{10}$, $X_{11}$')

## Feature Importance
st.subheader(':small_blue_diamond: Feature Importance')
st.text('사진')
st.markdown('XGB 모델에서는 Feature Importance가 클수록 해당 설명변수가 반응변수 예측에 미치는 영향이 크다고 해석함.')
st.markdown('각 반응변수의 변수 중요도 순서는 다음과 같음.')
st.markdown('- $Y_3$의 경우 $X_3$ > $X_1$ > $X_4$ > $X_{10}$ > $X_7$ > $X_6$ > $X_5$ > $X_8$')
st.markdown('- $Y_4$의 경우 $X_1$ > $X_3$ > $X_4$ > $X_7$ > $X_{10}$ > $X_5$ > $X_6$ > $X_8$')
st.markdown('- $Y_5$의 경우 $X_1$ > $X_3$ > $X_4$ > $X_7$ > $X_{10}$ > $X_5$ > $X_6$ > $X_{11}$')
st.markdown('- $Y_6$의 경우 $X_1$ > $X_3$ > $X_4$ > $X_6$ > $X_{10}$ > $X_{11}$')

## Feature Importance - Y3, Y4
st.subheader(':small_blue_diamond: Feature Importance - Y3, Y4')
st.text('사진')
###########################################################################################3
